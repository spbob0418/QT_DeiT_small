...
class Q_Block(nn.Module):
    ...
    def __init__(self, abits, wbits, gbits, qdtype, dim, num_heads, mlp_ratio=4., 
                #  qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()
        self.norm1 = norm_layer(dim)
        ######
        self.qact1 = QuantAct(abits, torch.int8)
        ...
        self.qact3 = QuantAct(abits, torch.int8)
        self.mlp = Mlp(
            abits, 
            wbits, 
            gbits,
...
class lowbit_VisionTransformer(nn.Module):
    ...
    def __init__(self, abits, wbits, gbits, qdtype, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.blocks = nn.Sequential(*[
            Q_Block(abits, wbits, gbits, qdtype, embed_dim, num_heads)
...
def qt_deit_small_patch16_224(pretrained=False, **kwargs):
    model = lowbit_VisionTransformer(
        abits=None , wbits=None, gbits=None, qdtype=torch.float32,
        patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
